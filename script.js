...
function updatePlayer() {
    ...
    if (isJumping && isGrounded()) {
        ...
        isJumping = false; // Disable jump until the player presses jump again
    }

    const newPosition = camera.position.clone().add(velocity);
    if (!checkCollisions(newPosition)) {
        camera.position.add(velocity); // Apply movement if no collision
    }
}
...
function animate() {
    ...
    updateChunks(); // Check if chunks need to be loaded/unloaded
